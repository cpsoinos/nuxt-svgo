...
// Expected path data after SVGO optimization (same source SVGs used in
// componentext.options.auto1/auto2 tests, so output is known-stable).
...
describe('autoImportPath: array of multiple paths', async () => {
  await setup({
  ...
  it('should render an svg from the first configured path', async () => {
    const html = await $fetch('/')
    expect(html).toContain(HOUSE_PATH)
  })

  it('should render an svg from the second configured path', async () => {
    const html = await $fetch('/')
    expect(html).toContain(STAR_PATH)
  })
})
